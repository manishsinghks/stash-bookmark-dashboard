...
export function TagInput({
  ...
  return (
    <div className="relative">
      <div
        className={cn(
          "flex min-h-9 flex-wrap items-center gap-1 rounded-lg border bg-transparent px-2 py-1.5 text-sm transition-colors",
        ...
        <div className="absolute top-full z-50 mt-1 w-full overflow-hidden rounded-lg border bg-popover shadow-lifted">
          {matched.map((tag) => (
            <button
              key={tag.id}
              type="button"
              className="flex w-full cursor-pointer items-center justify-between px-2.5 py-1.5 text-sm transition-colors hover:bg-accent"
              onMouseDown={(event) => {
                event.preventDefault();
                addTag(tag.name);
              }}
